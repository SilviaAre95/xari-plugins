...
5. **Dev test (project-shaped).** Unit tests belong to the deterministic gate; this stage exercises the change the way the product is actually used. Pick the method in this order:
...
7. **PR** (if `.cc-dev.yaml` `open_pr` is not false). Push the branch. Then check for an existing PR first — re-runs must converge to ONE PR, never two:
